  ...
  return new Promise((resolve, reject) => {
    this.discoveryClient.listConfigurations(params, (err, data) => {
      if (err) {
        ...
        const configs = data.result.configurations;
        for (let i = 0, size = configs.length; i < size; i++) {
          ...
          if (config.name === 'Default Configuration') {
            ...
            params.configurationId = config.configuration_id;
...
WatsonDiscoverySetup.prototype.createDiscoveryConfig = function(params) {
  return new Promise((resolve, reject) => {
    ...
    config_params.conversions = params.default_configuration.result.conversions;
    config_params.normalizations = params.default_configuration.result.normalizations;
    config_params.enrichments = params.default_configuration.result.enrichments;
    ...
    this.discoveryClient.createConfiguration(config_params, (err, data) => {
      if (err) {
        ...
        params.configurationId = data.configuration_id;
  ...
  return new Promise((resolve, reject) => {
    ...
    this.discoveryClient.createEnvironment(createParams, (err, data) => {
      if (err) {
        ...
        console.log('Created Discovery environment: ', data);
        params.environmentId = data.result.environment_id;
        resolve(params);
      }
    });
  });
...
WatsonDiscoverySetup.prototype.createDiscoveryCollection = function(params) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const createCollectionParams = {
      ...
      description: 'Discovery collection created by ask-k.',
      language_code: 'en_us'
    };
